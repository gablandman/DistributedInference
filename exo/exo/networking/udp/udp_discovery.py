...
class UDPDiscovery:
    ...
    async def task_broadcast_presence(self):
        if DEBUG_DISCOVERY >= 2:
            print("Starting unicast discovery task...")

        while True:
            for peer_id, (peer_host, peer_port) in self.known_peers.items():
                message = json.dumps(
                    {
                        "type": "discovery",
                ...
                if DEBUG_DISCOVERY >= 3:
                    print(f"Sending unicast to {peer_host}:{peer_port}: {message}")

                transport = None
                try:
                    # Pass all required parameters to the protocol
                    (
                        transport,
                        _,
                    ) = await asyncio.get_event_loop().create_datagram_endpoint(
                        lambda: UnicastProtocol(message, peer_host, peer_port),
                        local_addr=("0.0.0.0", 0),
                        family=socket.AF_INET,
                    )
                except Exception as e:
                    print(f"Error sending unicast to {peer_host}:{peer_port}: {e}")
                finally:
                    if transport:
                        try:
                            transport.close()
            ...
            )  # Periodically send the unicast message
    ...
    async def task_listen_for_peers(self):
        try:
            ...
            if DEBUG_DISCOVERY >= 2:
                print("Started listening for peer messages...")
        except Exception as e:
            if DEBUG_DISCOVERY >= 1:
