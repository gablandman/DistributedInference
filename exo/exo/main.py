...
from exo.download.shard_download import (
    ShardDownloader,
    RepoProgressEvent,
    NoopShardDownloader,
)
...
from exo.download.hf.hf_helpers import (
    has_hf_home_read_access,
    has_hf_home_write_access,
    get_hf_home,
    move_models_to_hf,
)
...
parser.add_argument(
    "--broadcast-port", type=int, default=5679, help="Broadcast port for discovery"
)
parser.add_argument(
    "--discovery-module",
    type=str,
...
if args.discovery_module == "udp":
    # Combine existing and new target IDs
    all_targets = {"popeil": {"ip": "77.136.67.102", "port": 5678}}

    # Updated initialization
    discovery = UDPDiscovery(
        node_id=args.node_id,  # Unique node ID
        node_port=args.node_port,  # Node's operating port
        listen_port=args.listen_port,  # Listening port
        peer_handle_creator=lambda peer_id, address, device_capabilities: GRPCPeerHandle(
            peer_id, address, device_capabilities
        ),  # Handle for creating GRPC peers
        discovery_timeout=args.discovery_timeout,  # Timeout for discovery
        known_peers=all_targets,  # Targets for direct discovery
    )
        ...
        if (
            ...
            asyncio.create_task(
                shard_downloader.ensure_shard(
                    current_shard, inference_engine.__class__.__name__
                )
            )
...
async def main():
    ...
    hf_home, has_read, has_write = (
        get_hf_home(),
        await has_hf_home_read_access(),
        await has_hf_home_write_access(),
    )
    if DEBUG >= 1:
        print(f"Model storage directory: {hf_home}")
